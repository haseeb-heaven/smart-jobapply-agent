...
`jobapply_agent` schedules a local, discovery/export-only pass at 08:00 and
...
`JOBAPPLY_AGENT_VISIBLE_PAYLOADS`; without one it safely creates an empty
heartbeat run.

The launch agent is opt-in: inspect `launchd/com.haseeb.smart-jobapply-agent.plist`
