 ...
 * It deliberately does not retain a browser binding as part of the key.
 */
export function startOrGetCodexSmartQueueDaemonHost(chromeBinding, options) {
  const configuration = daemonHostConfiguration(options);
  const key = runtimeKey(configuration);
  const existing = supervisedHosts.get(key);
  if (existing !== undefined) return existing;
  const host = startCodexSmartQueueDaemonHost(chromeBinding, options);
